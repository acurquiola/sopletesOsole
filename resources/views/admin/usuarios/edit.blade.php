			...
			<div class="col s12">
				...
				<form method="POST"  enctype="multipart/form-data" action="{{action('UserController@update', $user->id)}}" class="col s12 m8 offset-m2 xl10 offset-xl1">
					...
					<div class="row">
						...
						<div class="input-field col s4">
							<i class="material-icons prefix">keyboard_arrow_right</i>
							<input id="icon_prefix" type="text" class="validate" name="nombre"   value="{{$user->nombre}}" >
							<label for="icon_prefix">Nombre</label>
						</div>
						<div class="input-field col s4">
							<i class="material-icons prefix">keyboard_arrow_right</i>
							<input id="icon_prefix" type="text" class="validate" name="apellido"   value="{{$user->apellido}}" >
						...
						<div class="input-field col s4">
							<i class="material-icons prefix">keyboard_arrow_right</i>
							<input id="icon_prefix" type="email" class="validate" name="email"   value="{{$user->email}}" >
							<label for="icon_prefix">Email</label>
						</div>
						<div class="input-field col s4">
							<select name="tipo_usuario_id">
								@foreach($tipos as $t)
								<option value="{{ $t->id }}" @if($t->id == $user->tipo_usuario_id) selected @endif> {{ $t->nombre }}</option>
								@endforeach
							</select>
						</div>
						<div class="right">
